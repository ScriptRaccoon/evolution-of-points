...
export class Population {
    ...
    update(ctx, step, obstacles) {
        ...
        this.done =
            step >= this.stepRecord ||
            this.points.every(
                (point) => point.finished || point.dead
            );
    }
    ...
    drawInfoText(ctx) {
        ...
        ctx.textAlign = "right";
        ctx.fillText(
            "Success rate: " + Math.ceil(this.successRate) + "%",
            ctx.canvas.width - 10,
            ctx.canvas.height - 10
        );
    ...
    calculcateSuccessRate() {
        let finishers = this.points.filter(
            (point) => point.finished == true
        );
    ...
    calculateBestPoint() {
        ...
        for (let j = 1; j < this.size; j++) {
            if (
                this.points[j].fitness >
                this.points[bestIndex].fitness
            ) {
                bestIndex = j;
            }
        }
        ...
        if (this.bestPoint.finished) {
            this.stepRecord = Math.min(
                this.stepRecord,
                this.bestPoint.stepsToGoal
            );
        }
    }
    ...
    naturalSelection() {
        let totalSum = this.points.reduce(
            (partialSum, point) => partialSum + point.fitness,
            0
        );

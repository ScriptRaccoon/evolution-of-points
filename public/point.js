...
import { pointAlpha } from "./main.js";
...
export class Point {
    ...
    update(ctx, step, obstacles) {
        ...
        if (
            ...
            this.pos[0] + this.size > ctx.canvas.width ||
            this.pos[1] < this.size ||
            this.pos[1] + this.size > ctx.canvas.height
        ) {
            this.dead = true;
            return;
        }
        ...
        if (
            squareDistance(this.pos, goal.pos) <
            Math.pow(goal.size, 2)
        ) {
            this.finished = true;
    ...
    calculateFitness() {
        ...
        if (this.finished) {
            myFitness =
                100 + Math.pow(this.lifeSpan - this.stepsToGoal, 2);
        } else {
